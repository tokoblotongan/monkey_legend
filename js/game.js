// ============================================
// MONKEY LEGEND - GAME ENGINE (FINAL VERSION)
// ✅ Enter & klik tombol sekarang berfungsi
// ✅ King Kong sejajar Sun Wukong (bukan di bawah)
// ✅ Tidak ada error pada initGame()
// ============================================

// === CANVAS SETUP ===
var C = document.getElementById('c');
if (!C) {
  console.error('❌ <canvas id="c"> tidak ditemukan!');
  alert('Error: Canvas tidak ditemukan. Pastikan index.html memiliki <canvas id="c">.');
}
var X, W, H;
...
function updateQuest(type, amount = 1) {
  for (var i = 0; i < dailyQuests.length; i++) {
    var q = dailyQuests[i];
    if (q.done) continue;
    if (type === 'jump' && q.id === 'jump50') q.current += amount;
    else if (type === 'kill' && q.id === 'kill10') q.current += amount;
    else if (type === 'coin' && q.id === 'coin100') q.current += amount;
...
function renderQuestUI() {
  ...
  for (var i = 0; i < dailyQuests.length; i++) {
    ...
    html += '<div class="quest-item ' + cls + '">' +
            '<div class="quest-label">' + q.label + ': ' + q.current + '/' + q.target + '</div>' +
...
function makePlat(x, y, w, type) {
  return { x: x, y: y, w: w, h: PLAT_H, type: type || 'candy', coins: [] };
}
...
function updateGhost(g) {
  ...
  if (Math.abs(g.x - player.x) < g.size + PW * 0.4 &&
      Math.abs(g.y - player.y) < g.size + PH * 0.4 &&
      player.inv <= 0) {
  ...
  if (petState === 'normal') {
    ...
    for (var i = 0; i < platforms.length; i++) {
      ...
      for (var j = 0; j < pl.coins.length; j++) {
        ...
        if (Math.hypot(pet.x - c.x, pet.y - c.y) < 80) {
          c.collected = true;
          petPoints += 10;
          score += 50;
          sfxCoin();
...
function restartGame() {
  ...
  startGame(); // sama seperti startGame, karena kita reset semua
}
...
function loop() {
  ...
  // Only run logic if playing
  if (state !== 'play') return;
  frame++;
  window.frame = frame;

  // Check HP before update
  if (player.hp <= 0) {
    triggerGameOver();
    return;
  }
...
window.addEventListener('keydown', function(e) {
  ...
  } else if (key === 'a') { e.preventDefault(); keys['a'] = true; }
...
window.onload = function() {
  console.log('✅ Game loaded. Tekan ENTER atau klik "Mulai Petualangan" untuk mulai.');
};
